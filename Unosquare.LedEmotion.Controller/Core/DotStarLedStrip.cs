namespace Unosquare.LedEmotion.Controller.Core
{
    ...
    public class DotStarLedStrip
    {
        ...
        public static readonly byte[] EndFrame = new byte[] { 0xFF, 0xFF, 0xFF, 0xFF };
        ...
        internal readonly byte[] FrameBuffer; // Contains what needs to be written over the SPI channel
        private readonly byte[] PixelHolder = new byte[4]; // used heavily to manipulate pixels
        private readonly Dictionary<int, DotStarLedStripPixel> Pixels = new Dictionary<int, DotStarLedStripPixel>();
        ...
        public DotStarLedStrip(int ledCount = 60, int spiChannel = 1, int spiFrequency = SpiChannel.DefaultFrequency, bool reverseRgb = true)
        {
            // Basic properties
            LedCount = ledCount;
        ...
        public DotStarLedStripPixel GetPixel(int index)
        {
            ...
            lock (SyncLock)
            {
                if (Pixels.ContainsKey(index) == false)
                    Pixels[index] = new DotStarLedStripPixel(this, StartFrame.Length + (index * PixelHolder.Length));
